...
public class OrderDetailSQLQueries {
     ...
     */
	public static final String INSERT_DETAIL = "INSERT INTO order_details (order_status_id, total_amount) VALUES (?, ?)";
	
	/**
     * SQL query to retrieve all order details from the database.
     */
	public static final String GET_ALL_DETAILS = "SELECT od.id, od.total_amount, os.id as status_id, os.status_name "
     ...
     */
	public static final String GET_DETAIL_BY_ID = "SELECT od.id, od.total_amount, os.id as status_id, os.status_name "
            + "FROM order_details od "
            + "INNER JOIN order_status os ON od.order_status_id = os.id "
            + "WHERE od.id = ?";
     ...
     */
	public static final String GET_PRODUCTS_BY_ORDER_DETAIL_ID = "SELECT p.id, p.name, p.price, p.quantity, p.available, "
     ...
     */
	public static final String UPDATE_DETAIL = "UPDATE order_details SET order_status_id = ?, total_amount = ? WHERE id = ?";
	
	/**
     * SQL query to delete products associated with a specific order detail ID from the database.
     */
	public static final String DELETE_ORDER_DETAIL_PRODUCTS_BY_ORDER_ID = "DELETE FROM order_detail_products WHERE order_detail_id = ?";
	
	/**
     * SQL query to delete an order approval by order detail ID from the database.
     */
	public static final String DELETE_APPROVAL_BY_ORDER_ID = "DELETE FROM order_approvals WHERE order_detail_id = ?";
